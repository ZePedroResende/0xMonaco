...
contract ThePackage is ICar {
    ...
    function randomNumbaBaby(Monaco.CarData memory ourCar, Monaco.CarData calldata opps1, Monaco.CarData calldata opps2)
        private
        pure
        returns (uint256)
    {
    ...
    function boost(Monaco monaco, Monaco.CarData memory car, uint256 _amount) private {
    ...
    function oppStopper(Monaco.CarData calldata car, Monaco.CarData calldata opps) private pure returns (bool) {
        if (
            LIMITER <= opps.speed || (getGap(car, opps) == GapType.Large)
    ...
    function takeYourTurn(
        ...
        if (MADMAX < car.y && ourCarIndex != 0) {
            toShell = toShell || madMaxShelling(monaco, car, allCars[ourCarIndex - 1]);
        }

        (uint256 _boost, bool _toShell) = doLogic(monaco, allCars, ourCarIndex, eco);
    ...
    function decision(
        ...
        if (ourCarIndex == 0) {
            ...
            // got money to shell
            if (eco == GapType.Large || getDelta(car, firstCar) == GapType.Large) {
                toShell = true;
            }
    ...
    function doLogic(Monaco monaco, Monaco.CarData[] calldata allCars, uint256 ourCarIndex, GapType eco)
        private
        view
        returns (uint256 boostCounter, bool toShell)
    {
        Monaco.CarData calldata car = allCars[ourCarIndex];
        // main game logic, decide what to do based on position
        uint256 _boost;
        bool _shell;
        (_boost, _shell) = decision(ourCarIndex, car, allCars[0], allCars[1], allCars[2], eco);
        boostCounter += _boost;
    ...
    // spends surplus cash
    function checkBurn(Monaco monaco, uint256 ourCarIndex, Monaco.CarData calldata car, bool toShell)
        private
        view
        returns (uint256 _moreBoost, bool _toShell)
    {
        if (MID_GAME < car.y && car.y < MADMAX) {
            (_moreBoost, _toShell) = excess_burn(monaco, car, ourCarIndex, 1, toShell);
        } else if (MADMAX <= car.y && car.y < FLATOUT) {
            (_moreBoost, _toShell) = excess_burn(monaco, car, ourCarIndex, 2, toShell);
        } else if (FLATOUT <= car.y) {
            (_moreBoost, _toShell) = excess_burn(monaco, car, ourCarIndex, 2, toShell);
        }
    }
    ...
    function safeBoost(Monaco monaco, Monaco.CarData[] calldata allCars, uint256 ourCarIndex)
        private
        view
        returns (uint256 _moreBoost)
    {
    ...
    function madMaxShelling(Monaco monaco, Monaco.CarData calldata car, Monaco.CarData calldata opps)
        private
        view
        returns (bool)
    {
    ...
    function drs(Monaco.CarData calldata car, Monaco.CarData calldata opp, uint256 delta, uint256 fasterCase)
        private
        pure
        returns (uint256 moreBoost)
    {
        moreBoost = (opp.speed < car.speed) ? fasterCase : (opp.speed + delta - car.speed);
    }

    // determines if theres excess cash to spend
    function excess_burn(Monaco monaco, Monaco.CarData calldata car, uint256 place, uint256 multiplier, bool shelled)
        private
        view
        returns (uint256 moreBoost, bool toShell)
    {
    ...
    function max_bid(Monaco monaco, Monaco.CarData calldata car) private returns (uint256 moreBoost) {
        // if its cheap, fuggit
    ...
    function getEco(Monaco.CarData calldata car, Monaco.CarData calldata opp1, Monaco.CarData calldata opp2)
        private
        pure
        returns (GapType)
    {
