...
contract c000r is ICar {
    function takeYourTurn(
        ...
        // Win if possible.
        if (ourCar.y > 850 && ourCar.balance >= monaco.getAccelerateCost(1000 - (ourCar.y + ourCar.speed))) {
        ...
        if (allCars[0].y > 850) {
            ...
            if (ourCarIndex != 0) {
                ...
                if (
                    ourCar.balance >= shellCost && shellCost < 2000
                        && ((frontCarSpeed > ourCar.speed && frontCarSpeed > 4) || frontCarSpeed > 8)
                ) {
                    ourCar.balance -= uint24(monaco.buyShell(1));
                    shellCost = 15001;
                }
        ...
        if (ourCarIndex == 2) {
            if (allCars[1].balance < 200 || (allCars[0].y > 825 && allCars[1].y < 750)) {
                ...
                costToCatchUp =
                    monaco.getAccelerateCost(1 + allCars[1].speed + allCars[1].y - (ourCar.speed + ourCar.y));
        ...
        if (ourCarIndex == 1 && nextCarSpeed > 3 && ourCar.balance >= shellCost) {
            // if the car ahead of us is going fast and it's not really expensive to get em, smoke em.
            if (shellCost < 300 && (nextCarSpeed > (ourCar.speed + 6) || nextCarSpeed > 24)) {
                ourCar.balance -= uint24(monaco.buyShell(1));
            } else if (
                shellCost < 1000
        ...
        if (
            ourCar
                ...
                .balance > (allCars[0].y < 800 ? 2000 : 500) && ourCar.speed < 6
        ) {
            ...
            costToCatchUp != 0 && ourCar.balance > costToCatchUp && costToCatchUp < 1000
        ) {
            ...
            if (
                ourCar.balance > 200 && ourCar.balance > costToAccelerate
