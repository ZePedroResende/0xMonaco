...
contract MonacoTest is Test {
    ...
    event log_car(string key, string name, address val);

    Monaco monaco;
    address[CAR_LEN] cars;

    mapping(ICar => string) names;
        ...
        while (monaco.state() != Monaco.State.DONE) {
            ...
            for (uint256 abilityIdx = 0; abilityIdx <= uint256(Monaco.ActionType.SHIELD); ++abilityIdx) {
                currentTurn.usedAbilities[abilityIdx] =
                    monaco.getActionsSold(Monaco.ActionType(abilityIdx)) - currentTurn.usedAbilities[abilityIdx];
            }
            ...
            vm.writeLine("logs/gameLog.json", string.concat((firstLine) ? "" : ",", encodeJson(currentTurn)));
    ...
    function encodeJson(GameTurn memory turn) private view returns (string memory) {
    ...
    function encodeCarStats(GameTurn memory turn) private pure returns (string memory) {
        return string.concat(
            encodeBalance(turn),
            ",",
            encodeSpeed(turn),
    ...
    function encodeBalance(GameTurn memory turn) private pure returns (string memory) {
        return string.concat(
            '"balance":[',
    ...
    function encodeSpeed(GameTurn memory turn) private pure returns (string memory) {
        return string.concat(
            '"speed":[',
    ...
    function encodeY(GameTurn memory turn) private pure returns (string memory) {
        return string.concat(
            '"y":[', vm.toString(turn.y[0]), ",", vm.toString(turn.y[1]), ",", vm.toString(turn.y[2]), "]"
        );
    }

    function encodeShield(GameTurn memory turn) private pure returns (string memory) {
        return string.concat(
            '"shield":[',
    ...
    function encodeCosts(GameTurn memory turn) private pure returns (string memory) {
        return string.concat(
            '"costs":[',
    ...
    function encodeBought(GameTurn memory turn) private pure returns (string memory) {
        return string.concat(
            '"bought":[',
    ...
    function encodeUsedAbilities(GameTurn memory turn) private pure returns (string memory) {
        return string.concat(
            '"usedAbilities":[',
    ...
    function encodeBananas(GameTurn memory turn) private pure returns (string memory) {
        string memory bananas = '"bananas":[';
        uint256 len = turn.bananas.length;
        for (uint256 i = 0; i < len; ++i) {
            bananas = string.concat(bananas, (i > 0) ? "," : "", vm.toString(turn.bananas[i]));
        }
    ...
    function encodeCars() private view returns (string memory) {
        return string.concat(
            '"cars":["', vm.toString(cars[0]), '","', vm.toString(cars[1]), '","', vm.toString(cars[2]), '"]'
        );
    }

    function encodeCars(GameTurn memory turn) private view returns (string memory) {
        return string.concat(
            '"cars":["',
